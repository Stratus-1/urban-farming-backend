...
def score_assessment(payload: InspectionAssessment) -> InspectionAssessmentResult:
    sunlight = 30 if payload.sunlight_hours >= 6 else 22 if payload.sunlight_hours >= 4 else 10 if payload.sunlight_hours >= 2 else 0
    water = {"reliable": 25, "limited": 12, "none": 0}[payload.water_access]
    space = 20 if payload.usable_space_m2 >= 10 else 15 if payload.usable_space_m2 >= 5 else 8 if payload.usable_space_m2 >= 2 else 3
